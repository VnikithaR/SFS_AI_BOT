...
##########################
# UG COURSES RELATED CODE
##########################

class ActionGetUGCourses(Action):
    def name(self) -> Text:
        return "action_get_ug_courses"

    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:

        course_doc = db.courses.find_one({"category": "courses/ug_courses"})
        if not course_doc:
            dispatcher.utter_message(text="Sorry, I couldn't fetch the UG courses list right now.")
            return []

        course_list = course_doc.get("ug_courses", [])
        if not course_list:
            dispatcher.utter_message(text="No UG courses found in the database.")
            return []

        response = "📚 Available UG Courses:\n" + "\n".join(f"- {course}" for course in course_list)
        dispatcher.utter_message(text=response)
        return []

class ActionFetchUGCourseFee(Action):
    def name(self) -> Text:
        return "action_fetch_ug_course_fee"
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        key = next((k for k in db.ug_courses_fee_details.keys() if db.ug_courses_fee_details[k]["course_name"].lower() == course_name.lower()), None)

        if not key:
            dispatcher.utter_message(text=f"Sorry, I couldn't find fee details for '{course_name}'.")
            return []

        course = db.ug_courses_fee_details[key]
        annual_fee = course.get("annual_fee", {})
        semester_fee = course.get("semester_fee", {})
...
class ActionFetchUGSyllabus(Action):
    def name(self) -> Text:
        return "action_fetch_ug_syllabus"
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        file_path = "bot_data/courses/ug_courses/ug_syllabus.json"
...
##########################
# NAAC ACCREDITATION RELATED CODE
##########################
